    ...
    try:
        with db_helper.session_scope() as db_session:
            ...
            if message.reply_to_message:
                ...
                await send_message_to_admin(bot, chat_id, f"User {reporting_user_mention} reported {reported_user_mention} in chat {chat_helper.get_chat_mention(chat_id)}. Total reports: {report_count}. \nReported message: {message.reply_to_message.text}")
                ...
                if report_count >= number_of_reports_to_ban:
                    ...
                    await send_message_to_admin(bot, chat_id, f"User {reported_user_mention} has been banned in chat {chat_helper.get_chat_mention(chat_id)} due to {report_count} reports.")
                ...
                if report_count >= number_of_reports_to_warn:
                    ...
                    await send_message_to_admin(bot, chat_id, f"User {reported_user_mention} has been warned and muted in chat {chat_helper.get_chat_mention(chat_id)} due to {report_count} reports.")
    except Exception as error:
        logger.error(f"Error: {traceback.format_exc()}")
